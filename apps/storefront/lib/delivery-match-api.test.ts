...
describe('fetchDeliveryMatch', () => {
  it('dentro da zona: devolve o payload tipado', async () => {
    ...
    const result = await fetchDeliveryMatch('hamburgueria-da-vila', -29.6, -51.17);
  ...
  it('fora da zona: devolve o discriminante false', async () => {
    ...
    expect(await fetchDeliveryMatch('hamburgueria-da-vila', 0, 0)).toEqual({ withinZone: false });
  });

  it('resposta não-200: devolve null, nunca lança', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => ({ ok: false, json: async () => ({}) })));

    expect(await fetchDeliveryMatch('hamburgueria-da-vila', 0, 0)).toBeNull();
  });
  ...
  it('erro de rede: devolve null, nunca lança', async () => {
    ...
    await expect(fetchDeliveryMatch('hamburgueria-da-vila', 0, 0)).resolves.toBeNull();
  });
