...
describe('fetchDeliveryMatch', () => {
  it('dentro da zona: devolve o payload tipado', async () => {
    ...
    const result = await fetchDeliveryMatch('hamburgueria-da-vila', '93610-000', '1684');
  ...
  it('fora da zona: devolve o discriminante false', async () => {
    ...
    expect(await fetchDeliveryMatch('hamburgueria-da-vila', '93610-000', '1684')).toEqual({ withinZone: false });
  });

  it('resposta não-200: devolve null, nunca lança', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => ({ ok: false, json: async () => ({}) })));

    expect(await fetchDeliveryMatch('hamburgueria-da-vila', '93610-000', '1684')).toBeNull();
  });
  ...
  it('erro de rede: devolve null, nunca lança', async () => {
    ...
    await expect(fetchDeliveryMatch('hamburgueria-da-vila', '93610-000', '1684')).resolves.toBeNull();
  });
