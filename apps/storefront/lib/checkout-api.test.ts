...
describe('createOrder', () => {
  ...
  it('201 pix: devolve status created com orderId/totalCents/pix', async () => {
    ...
    expect(resultado).toEqual({ status: 'created', orderId: 'order-1', trackingToken: 'tracking-1', totalCents: 7380, cashbackUsedCents: 0, fulfillmentType: 'delivery', fulfillmentDeadlineAt: deadline, paymentMethod: 'pix', pix });
  ...
  it('201 cash_on_delivery: devolve status created com changeForCents, sem pix', async () => {
    ...
    const resultado = await createOrder('hamburgueria-da-vila', body, { accessToken: 'token-x' });
    expect(resultado).toEqual({ status: 'created', orderId: 'order-2', trackingToken: 'tracking-2', totalCents: 7380, cashbackUsedCents: 0, fulfillmentType: 'delivery', fulfillmentDeadlineAt: deadline, paymentMethod: 'cash_on_delivery', changeForCents: 8000 });
  });
  ...
  it('201 card_on_delivery: devolve status created sem pix nem changeForCents', async () => {
    ...
    const resultado = await createOrder('hamburgueria-da-vila', body, { accessToken: 'token-x' });
    expect(resultado).toEqual({ status: 'created', orderId: 'order-3', trackingToken: 'tracking-3', totalCents: 7380, cashbackUsedCents: 0, fulfillmentType: 'delivery', fulfillmentDeadlineAt: deadline, paymentMethod: 'card_on_delivery' });
  });
