...
- **`request.ip` atrás de proxy sem `trust proxy` configurado no Express.** Registrado no Épico 5, commit 2 (rate limit do storefront), mas vale IGUAL pro rate limit de OTP e de upload de imagem, que já usam a mesma convenção. Hoje `app.set('trust proxy', ...)` não é chamado em lugar nenhum: em produção atrás do proxy da Vercel/Cloudflare, `request.ip` é o IP do PROXY, não o do cliente — todos os clientes colapsam numa chave só de rate limit e um visitante legítimo derruba os outros. Não morde em dev (conexão direta, IP real). Fechar antes do go-live do piloto: `trust proxy` com o número de hops certo (nunca `true` cego, que deixa qualquer um forjar `X-Forwarded-For` e escapar do limite).
