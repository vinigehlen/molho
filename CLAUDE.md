...
**Mesclado no `main` (2026-09-02). Código completo, gate verde.** A tela `/login`, o `staff-auth` (access token em memória + refresh no cookie `__Host-molho_refresh`), o seletor de tenant, o bootstrap autenticado de `/gestor/*`, o arm/disarm do SSE e a remoção do stub `/dev-login` já estão na branch principal. **O que falta é deploy**, não código: a promoção ao domínio do gestor depende do ambiente real (Épico 9c). A fonte detalhada é `docs/09b-auth-backoffice.md`; este resumo substitui o desenho histórico do item 9b abaixo quando houver divergência.
...
9. ✅ Gestor de pedidos realtime + push/som + fila offline + **login real de staff (9b)**. Backend e frontend mesclados no `main`, gate verde. Resta o **Épico 9c** (infra real: `molho.live`, Neon `sa-east-1`, Upstash SP, 2 máquinas Fly GRU) — pré-requisito de deploy, não de código.
