...
- **URL pública de leitura da foto de produto (`Product.imageKey` → URL renderizável).** Adiado no Épico 4, commit 6 (upload via presigned PUT no R2) — escopo daquele commit é só WRITE. **Meia resolução no Épico 5, commit 2:** o MECANISMO existe (`apps/api/src/storage/public-url.ts`, `resolvePublicImageUrl(imageKey, S3_PUBLIC_URL)`), mora no servidor de propósito (front nunca monta URL de bucket) e degrada pra `imageUrl: null` sem quebrar o contrato — o card cai no placeholder do tema. **Continua aberto:** `S3_PUBLIC_URL` segue vazia em `.env.local`/`.env.example` — falta a decisão de INFRA de como expor leitura do bucket `molho-uploads` (bucket público, domínio customizado do R2, ou Cloudflare Images/Worker na frente). Enquanto isso, nenhuma foto de produto renderiza. Fechar até o 13b (tema/logo/capa), que não tem como degradar pra placeholder — logo e capa da loja são o visual da marca.
