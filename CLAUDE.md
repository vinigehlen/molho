...
- **URL pública de leitura da foto de produto (`Product.imageKey` → URL renderizável).** Adiado no Épico 4, commit 6 (upload via presigned PUT no R2) — escopo daquele commit é só WRITE. `S3_PUBLIC_URL` está vazia em `.env.local`/`.env.example`: falta decidir e configurar o acesso de leitura do bucket `molho-uploads` (bucket público, domínio customizado do R2, ou Cloudflare Images/Worker na frente) antes que qualquer front consiga exibir a foto pelo `imageKey` salvo. Fica pro Épico 5 (storefront: menu, carrinho) ou 13b (tema/logo/capa), o que precisar renderizar imagem primeiro.
